...
var (
	...
	Second  = Registry.Derive("s", unit.Scalar(9192631770).Div(natural.Caesium))
	Hertz   = Registry.Derive("Hz", Second(1).Recip())
	Caesium = unit.MustConvert(natural.Caesium, Hertz)

	Meter = Registry.Derive("m", natural.C.Mul(Second)(1).DivN(299792458))
	C     = unit.MustConvert(natural.C, Meter.Div(Second))

	Kilogram = Registry.Derive("kg", natural.H.Mul(Second).Div(Meter.Pow(2))(float64(1)/6.62607015e-34))
	Newton   = Registry.Derive("N", Kilogram.Mul(Meter).Div(Second.Pow(2)))
	Joule    = Registry.Derive("J", Meter.Mul(Newton))
	...
	Steradian = Registry.Derive("sr", Meter.Pow(2).Div(Meter.Pow(2)))
	...
	Gram       = Registry.Derive("g", Kilogram.Mul(unit.Scalar(1000)))
	...
	Liter      = Registry.Derive("L", Centi(Meter).Pow(3)(1000))
	Lumen      = Registry.Derive("lm", Candela.Mul(Steradian))
	Lux        = Registry.Derive("lx", Lumen.Div(Meter.Pow(2)))
	Ohm        = Registry.Derive("Ω", Volt.Div(Ampere), "Ohm")
	Pascal     = Registry.Derive("Pa", Newton.Div(Meter.Pow(2)))
	Radian     = Registry.Derive("rad", Meter.Div(Meter))
	Siemens    = Registry.Derive("S", Ampere.Div(Volt))
	Sievert    = Registry.Derive("Sv", Joule.Div(Kilogram))
	Tesla      = Registry.Derive("T", Volt.Mul(Second).Div(Meter.Pow(2)))
